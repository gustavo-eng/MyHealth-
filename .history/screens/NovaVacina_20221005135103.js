...
const NovaVacina = (props) => {
    ...
    // teste de comparacao 
    let d1 = new Date()
    let d2 = new Date(listaVacinas[0].proximaVacina.toString())
    console.log(` transformando em string em data ${listaVacinas[0].proximaVacina.toString()}`)

    if(d2.getDate().toLocaleString('pt-br') < d1.getDate().toLocaleString('pt-br')){
        c
        onsole.log(` ${d2.getDate().toLocaleString('pt-br')} e menor que ${d1.getDate()}`)
    } else {
        console.log(` ${d2.getDate()} e menor que ${d1.getDate()}`)
    } 
