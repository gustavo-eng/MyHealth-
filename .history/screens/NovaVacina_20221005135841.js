...
const NovaVacina = (props) => {
    ...
    // teste de comparacao 
    let d1 = new Date('08/10/2022')
    let d2 = new Date((listaVacinas[0].proximaVacina).toLocaleLowerCase())
    //console.log(` transformando em string em data ${listaVacinas[0].proximaVacina.toString()}`)
 
    if(d1.getDate().toLocaleString('pt-br') === d2.getDate().toLocaleString('pt-br')) {
         console.log('iguais')
    } 
    if(d1.getDate().toLocaleString('pt-br') < d2.getDate().toLocaleString('pt-br')) {
        console.log('dentro de menor ')
    }
