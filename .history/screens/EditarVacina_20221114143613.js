...
const EditarVacina = (props) => {
    const [selected, setSelected] = useState(useSelector((state) => state.vacina.vacina))
    const [modalVisible, setModalVisible] = useState(false) 
    const {setAlterou, alterou} = props.route.params
    
    //Storage 
    const [urlFoto, setUrlFoto] = useState(props.route.params?.urlImage)
    ...
    //let NomeVacina = useSelector((state) => state.vacina.vacina) 
